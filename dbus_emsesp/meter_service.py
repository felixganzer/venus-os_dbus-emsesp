...
class HeatPumpMeterService:
    def __init__(self, config, electrical, version, category):
        ...
        self.update_index = 0
        self.service = VeDbusService(config["service_name"], register=False)
        ...
        add("/Ac/L1/Power", 0.0, gettextcallback=fmt("W", 0))
        add("/Ac/Voltage", self.voltage, gettextcallback=fmt("V", 1))
        add("/Ac/L1/Voltage", self.voltage, gettextcallback=fmt("V", 1))
        add("/Ac/Current", 0.0, gettextcallback=fmt("A", 2))
        add("/Ac/L1/Current", 0.0, gettextcallback=fmt("A", 2))
        add("/Ac/PowerFactor", 1.0)
        add("/Ac/L1/PowerFactor", 1.0)
        add("/Ac/Energy/Forward", 0.0, gettextcallback=fmt("kWh", 3))
        add("/Ac/L1/Energy/Forward", 0.0, gettextcallback=fmt("kWh", 3))
        add("/Ac/Energy/Reverse", 0.0, gettextcallback=fmt("kWh", 3))
        add("/Ac/L1/Energy/Reverse", 0.0, gettextcallback=fmt("kWh", 3))
        ...
        add("/Position", int(electrical.get("position", 1)))
        add("/PhaseSetting", int(electrical.get("phase_setting", 1)))
        add("/DeviceType", 0)
        add("/ErrorCode", 0)
